    ...
    // some block
      ...
      withCredentials([string(credentialsId: 'sonar', variable: 'sonarLogin')]) {
        ...
        //sh "${sonarqubeScannerHome}/bin/sonar-scanner -e -Dsonar.host.url=http://ecmserver:9000 -Dsonar.login=${sonarLogin} -Dsonar.projectName=gs-gradle -Dsonar.projectVersion=${env.BUILD_NUMBER} -Dsonar.projectKey=GS -Dsonar.sources=complete/src/main/ -Dsonar.tests=complete/src/test/ -Dsonar.language=java -Dsonar.java.binaries=."
	      sh "${sonarqubeScannerHome}/bin/sonar-scanner -e -Dsonar.host.url=http://ecmserver:9000 -Dsonar.login=${sonarLogin} -Dsonar.sourceEncoding=UTF-8 -Dsonar.tests=src/test -Dsonar.projectKey=ECMSampleWebApplication -Dsonar.java.binaries=target/classes -Dsonar.sources=src/main -Dsonar.projectName=ECMSampleWebApplication -Dsonar.projectVersion=${env.BUILD_NUMBER}  -Dsonar.language=java -Dsonar.java.binaries=."
